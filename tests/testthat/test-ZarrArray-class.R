...
test_that("ZarrArray()", {
    ...
    expect_identical(matrixClass(A), "_ZarrMatrix")
    expect_identical(as(A, "_ZarrMatrix"), A)  # no-op

    A2 <- writeZarrArray(sqrt(t(A[ , , 1]) + 1), chunkdim=c(5, 8))
    expect_true(is(A2, "_ZarrMatrix"))
    ...
    expect_identical(as(A2, "_ZarrArray"), A2)  # no-op
})
